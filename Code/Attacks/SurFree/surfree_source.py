...
class SurFree(MinimizationAttack):
    ...
    def __init__(
        ...
        """
        # Attack Parameters
        self._criterion_is_adversarial = None
        self.theta_max = None
    ...
    def _get_candidates(self, originals: ep.Tensor, best_advs: ep.Tensor) -> ep.Tensor:
        ...
        """
        epsilons = ep.zeros(ep.astensor(originals), len(originals))
        direction_2 = ep.zeros_like(ep.astensor(originals))
            ...
            for i, eps_i in enumerate(epsilons):
                if eps_i == 0:
                    self._directions_ortho[i] = ep.concatenate(
                        (self._directions_ortho[i], direction_2[i].expand_dims(0)), axis=0)
                    if len(self._directions_ortho[i]) > self.n_ortho + 1:
                        self._directions_ortho[i] = ep.concatenate(
                            (self._directions_ortho[i][:1], self._directions_ortho[i][self.n_ortho:]))
    ...
    def _get_evolution_function(self, originals: ep.Tensor, best_advs: ep.Tensor, direction2: ep.Tensor) -> Callable[
        [ep.Tensor], ep.Tensor]:
        try:
            best_advs = best_advs.numpy()
        except AttributeError:
        ...
        try:
            ...
            pass
        distances = np.linalg.norm(best_advs - originals)
        direction1 = (best_advs - originals).flatten() / distances
        direction1 = direction1.reshape(originals.shape)
        distances = atleast_kd(distances, direction1.ndim)
        try:
            direction1 = direction1.numpy()
            direction2 = direction2.numpy()
        except AttributeError:
            pass

        def _add_step_in_circular_direction(degree: ep.Tensor) -> ep.Tensor:
            degree = atleast_kd(degree, direction1.ndim).raw * np.pi / 180
            results = np.cos(degree) * direction1 + np.sin(degree) * direction2
            results = (originals + ep.astensor(results * distances * np.cos(degree))).clip(0, 1)
    ...
    def _binary_search_on_alpha(
        ...
        while step < self._BS_max_iteration and over_gamma.any():
            ...
            is_adv_opp = self._is_adversarial(mid_opp).numpy()

            lower = ep.where(over_gamma * is_adv, mid_bound, lower)
            lower = ep.where(over_gamma * is_adv.logical_not().numpy() * check_opposite * is_adv_opp, -mid_bound, lower)
            upper = ep.where(over_gamma * is_adv.logical_not().numpy() * check_opposite * is_adv_opp, - upper, upper)
    ...
    def _set_cos_sin_function(self, v: ep.Tensor) -> None:
        ...
        elif isinstance(v.raw, np.ndarray):
...
class Basis:
    def __init__(self, originals: ep.Tensor, random_noise: str = "normal", basis_type: str = "dct", **kwargs: Any):
        ...
        elif isinstance(self._originals.raw, np.ndarray):
    ...
    def dct2_8_8(self, image: ep.astensor, mask: ep.astensor) -> ep.astensor:
        ...
        for i in np.r_[:imsize[2]:8]:
            for j in np.r_[:imsize[3]:8]:
                dct_i_j = self._f_dct2(image[:, :, i:(i + 8), j:(j + 8)].numpy())
