...
if __name__ == '__main__':
    # Settings
    datasets = ['cifar']  # mnist or cifar
    n_particles = [5]
    n_nodes = [10]
    experiment_ids = list(range(5))
    max_queries = 25000
    distribution_schemes = ['rr']  # rr or mrr or dbl2 or dbe or rdbe
    history_len = 20
                ...
                for particles in n_particles:
                    for distribution_scheme in distribution_schemes:
                        ...
                        if distribution_scheme == 'rr':
                            ...
                            scheme = ResettingEmbeddedDistanceBasedDistributionScheme(None, n_nodes=nodes,
                                                                                      dataset=dataset,
                                                                                      history_len=history_len)
                            notify = True
                        else:
                        ...
                        with open(f'result_images/pso_bba_{dataset}_{experiment_id}.npy', 'wb') as f:
                            np.save(f, attack.swarm.best_position)
