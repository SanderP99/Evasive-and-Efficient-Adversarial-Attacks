    ...
    for i in range(n_experiments):
        for n_nodes in [1, 5]:
            ...
            random_inits = mnist.test_data[
                np.array(targets)[np.random.choice(len(targets), size=5, replace=False)]]
            distribution_scheme = RoundRobinDistribution()
            attack = DistributedBiasedBoundaryAttack(n_particles=5, model=bb_model,
                                                     target_img=x_orig,
            ...
            with open(output_file, 'a') as file:
                writer = csv.writer(file)
                writer.writerow(
                    [experiment.name, experiment.y_orig, experiment.y_target, 5, n_nodes, attack.swarm.best_fitness,
