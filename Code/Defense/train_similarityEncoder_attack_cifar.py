    ...
    for d in range(num_classes):
        for i in range(n):
            ...
            for _ in range(10):
                ...
                s = np.random.normal(mu, sigma, 32 * 32 * 3)
                s = s.reshape((32, 32, 3))
                s = np.absolute(s)
...
def create_base_network(input_shape):
    ...
    model.add(Dense(128, name='dense_encode'))  # 256 is the encoding dim in the stateful detection paper
...
print('data loaded')
x_train = x_test[1000:3000]
y_train = y_test[1000:3000]
...
base_network.save('CIFARAttackencoder.h5')
base_network.save_weights('CIFARAttackencoderweights.h5')
