...
if __name__ == '__main__':
    ...
    for i in range(1000):
        ...
        if i % 20 == 0:
            ...
            ax[0].imshow(target_image + swarm.particles[0].personal_best_position.reshape((28, 28, 1)), cmap=plt.get_cmap('gray'))
            ax[1].imshow(target_image + swarm.particles[-1].personal_best_position.reshape((28, 28, 1)), cmap=plt.get_cmap('gray'))
            ax[2].imshow((swarm.particles[0].position - swarm.particles[-1].position).reshape((28, 28, 1)), cmap=plt.get_cmap('gray'))
            prediction = np.argmax(model.predict(target_image + swarm.particles[0].personal_best_position.reshape((1, 28, 28, 1))))
            print(prediction)
            plt.show()
